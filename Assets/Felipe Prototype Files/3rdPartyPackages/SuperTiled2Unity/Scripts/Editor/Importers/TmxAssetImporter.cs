...
namespace SuperTiled2Unity.Editor
{
    ...
    public partial class TmxAssetImporter : TiledAssetImporter
    {
        ...
        private void ProcessMapLayers(GameObject goParent, XElement xMap)
        {
            ...
            var elementsInCustomOrder = GetElementsInCustomOrder(xMap);
            foreach (XElement xNode in elementsInCustomOrder)
            {
                ...
                using (SuperImportContext.BeginLayerIgnoreMode(ignoreMode))
                {
                    if (xNode.Name == "layer")
                    {
                        if (IsCustomPropertyLayer(xNode))
                        {
                            ProcessCustomTileLayer(xNode);
                        }
                        else
                        {
